...
namespace EmeraldAI
{
    ...
    public class EmeraldAISystem : MonoBehaviour
    {
        ...
        public float MinMeleeAttackSpeed = 0;
        public float MaxMeleeAttackSpeed = 0;
        public float MinRangedAttackSpeed = 0;
        public float MaxRangedAttackSpeed = 0;
        ...
        IEnumerator SwitchCurrentWeaponType(string WeaponTypeName)
        {
            ...
            if (WeaponTypeName == "Melee")
            {
                ...
                AttackSpeed = Random.Range(MinMeleeAttackSpeed, MaxMeleeAttackSpeed + 0.1f);
            ...
            else if (WeaponTypeName == "Ranged")
            {
                ...
                AttackSpeed = Random.Range(MinRangedAttackSpeed, MaxRangedAttackSpeed + 0.1f);
        ...
        void RotateAIStationary()
        {
            ...
            else //Non-Comabt rotations
            {
                ...
                {
                    if (!TurnDirectionMet && !LockTurning && CombatStateRef == CombatState.NotActive ||
                        !TurnDirectionMet && CombatStateRef == CombatState.Active)
                    {
                        IsTurning = false;
                        IsTurningLeft = false;
        ...
        void CreateEmeraldProjectile()
        {
            ...
            {
                Debug.LogError("There's a missing Ability Object in one of the Ability Objects slots of " +
                               gameObject.name +
                               ". Please go to AI's Settings>Combat>Damage Settings>Abilities and fill in any empty Ability Object slots.");
                return;
            }
                ...
                if (AdjustedAngle <= 180 && !EmeraldLookAtComponent.LookAtInProgress)
                {
                             ...
                             SupportAbilities.Count > 0 && !HealingCooldownActive) //Support
                    {
                                 ...
                                 m_EmeraldAIAbility.SupportType == EmeraldAIAbility.SupportTypeEnum.Instant)
                        {
                            ...
                            {
                                CombatTextSystem.Instance.CreateCombatTextAI(m_EmeraldAIAbility.AbilitySupportAmount,
                                    HitPointTransform.position, false, true);
                            }
                        }
                             ...
                             SummoningAbilities.Count > 0 && TotalSummonedAI < MaxAllowedSummonedAI) //Summon
                    {
                        ...
                        GameObject SummoningEffect = EmeraldAIObjectPool.SpawnEffect(m_EmeraldAIAbility.SummonEffect,
                            m_SummonedAI.transform.position, Quaternion.identity,
                            m_EmeraldAIAbility.AbilityEffectTimeoutSeconds);
                    ...
                    if (AdjustedAngle <= MaxDamageAngle || WeaponTypeRef == WeaponType.Ranged)
                    {
                        if (TargetTypeRef == TargetType.Player)
                        {
                            ...
                            if (!CriticalHit || CriticalHitSounds.Count == 0)
                            {
                                ...
                                //EmeraldEventsManagerComponent.PlayAttackSound();
                            }
