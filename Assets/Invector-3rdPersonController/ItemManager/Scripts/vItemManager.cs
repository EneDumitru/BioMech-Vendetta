    ...
    public partial class vItemManager : vMonoBehaviour, IActionReceiver
    {
        ...
        public virtual void AutoEquipItem(vItem item, int indexArea, bool autoEquip = false, bool ignoreItemAnimation = true)
        {
            ...
            if (inventory.equipAreas != null && inventory.equipAreas.Length > 0 && indexArea < inventory.equipAreas.Length)
            {
                ...
                else if (validSlot == null && autoEquip && inventory.equipAreas[indexArea].currentEquippedSlot && inventory.equipAreas[indexArea].currentEquippedSlot.item != null)
                {
                    //DropItem(inventory.equipAreas[indexArea].currentEquippedSlot.item);
                    DestroyItem(inventory.equipAreas[indexArea].currentEquippedSlot.item);
                    validSlot = inventory.equipAreas[indexArea].currentEquippedSlot;
                }
            ...
            //    for (int i = 0; i < inventory.equipAreas[indexArea].equipSlots.Count; i++)
            //    {
            //        if (inventory.equipAreas[indexArea].equipSlots[i].item)
            //        {
            //            Debug.Log("Slot: " + i + " " + inventory.equipAreas[indexArea].equipSlots[i].item.name);
            //        }
            //    }

            //    if (validSlot == null && indexArea == 0)
            //    {
            //        temporarilyIgnoreItemAnimation = true;
            //        if (inventory.equipAreas[indexArea].equipSlots[1].item)
            //        {
            //            DropItem(inventory.equipAreas[indexArea].equipSlots[1].item);
            //        }

            //        temporarilyIgnoreItemAnimation = false;
            //        autoEquip = true;
            //        validSlot = inventory.equipAreas[indexArea].equipSlots.Find(slot => slot.isValid && slot.item == null && slot.itemType.Contains(item.type));
            //    }
        ...
        public virtual void DropItem(vItem item, int amount)
        {
            ...
            if (item.dropObject != null)
            {
                ...
                StartCoroutine(ActivateObject(dropObject, 1));
        ...
        private IEnumerator ActivateObject(GameObject obj,float delayTime)
        {
