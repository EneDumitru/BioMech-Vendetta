...
namespace Invector.vCamera
{
    ...
    public class vThirdPersonCamera : MonoBehaviour
    {
        ...
        private float screenshakeIntensity = 0.07f;
        ...
        //    shakeScreen = true;
        //}
        public void ShakeScreenOnShoot(vShooterWeapon shooterWeapon)
        {
            screenshakeIntensity = Mathf.Clamp(shooterWeapon.recoilRight / shooterWeapon.recoilUp, 0.05f, 0.5f)/25;
            screenshakeDuration = 0.17f;
            shakeScreen = true;
        }

        public void ShakeScreenOnDamage(vDamage damage)
        {
            screenshakeIntensity = Mathf.Clamp01(damage.damageValue / 500)/2;
        ...
        public virtual void RotateCamera(float x, float y)
        {
            ...
            if (!currentState.cameraMode.Equals(TPCameraMode.FixedAngle))
            {
                ...
                if (!lockTarget)
                {
                    // -------- Sensitivity (compute once) --------
                    bool usingJoy = vInput.instance.inputDevice == InputDevice.Joystick;
                    float sensScalar = 3f * (SettingsManager.Instance.sensitivity / 100f);
                    float sensX = (usingJoy ? currentState.xMouseSensitivity * joystickSensitivity : currentState.xMouseSensitivity) * sensScalar;
                    float sensY = (usingJoy ? currentState.yMouseSensitivity * joystickSensitivity : currentState.yMouseSensitivity) * sensScalar;

                    // -------- Apply deltas (invert Y at the delta level) --------
                    float dx = x * sensX;
                    float dy = (SaveManager.Instance.saveData.invertedY ? y : -y) * sensY;

                    mouseX += dx;
                    mouseY += dy;

                    // Keep movementSpeed aligned with what we actually applied
                    movementSpeed.x = dx;
                    movementSpeed.y = dy;

                    CheckCameraIsRotating();
                    bool isAlignedWithTarget = (transform.forward - currentTarget.forward).magnitude <= 0.5f;
            ...
            else
            {
                // Fixed rotation mode
                smoothCameraRotation = initialCameraRotation;
                float _x = lerpState.fixedAngle.x;
                float _y = lerpState.fixedAngle.y;
        ...
        public virtual void CameraMovement(bool forceUpdate = false)
        {
            ...
            if (shakeScreen)
            {
                ...
                if (damageTimer < screenshakeDuration)
                {
                    ...
                    current_cPos += screenShakeOffset;
                }
                else
                {
                    shakeScreen = false;
