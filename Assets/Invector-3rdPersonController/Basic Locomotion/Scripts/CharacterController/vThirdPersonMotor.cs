...
namespace Invector.vCharacterController
{
    ...
    public class vThirdPersonMotor : vCharacter, vIAnimatorStateInfoController
    {
        ...
        private float _moveSpeedBonusPercentage;
        #endregion
        ...
        protected virtual void Awake()
        {
            ...
            foreach (var skillData in SkillsManager.Instance.SkillsData)
            {
                ...
                {
                    if (skillData.SupportType == SkillData.SupportSkillType.RegenerationStamina)
                    {
                        staminaRecovery += skillData.BonusValue / 100 * staminaRecovery;
                    }
                    
                    if (skillData.SupportType == SkillData.SupportSkillType.MoveSpeedHpBelow)
                    {
                        _moveSpeedBonusPercentage = skillData.BonusValue/100;
                    }
                }
            }
        ...
        public override void TakeDamage(vDamage damage)
        {
            ...
            PS5TrophyManager.Instance.IncreaseProgressStat(TrophyEvents.OnDamageTaken, TrophyParams.DamageTaken, TrophyID.MARTYRS_ENDURANCE, (int)damage.damageValue);
        ...
        {
            if (isLowHp && _moveSpeedBonusPercentage > 0)
            {
                if (isCrouching)
                {
                    var crouchSpeedBonus = _moveSpeedBonusPercentage * speed.crouchSpeed;
                    moveSpeed = Mathf.Lerp(moveSpeed, speed.crouchSpeed+crouchSpeedBonus, speed.movementSmooth * Time.fixedDeltaTime);
                    return;
                }

                if (speed.walkByDefault || alwaysWalkByDefault)
                {
                    var runningSpeedBonus = _moveSpeedBonusPercentage* speed.runningSpeed;
                    var walkSpeedBonus = _moveSpeedBonusPercentage * speed.walkSpeed;
                    
                    moveSpeed = Mathf.Lerp(moveSpeed, isSprinting ? speed.runningSpeed+runningSpeedBonus : speed.walkSpeed+walkSpeedBonus,
                        speed.movementSmooth * Time.fixedDeltaTime);
                }
                else
                {
                    var runningSpeedBonus = _moveSpeedBonusPercentage * speed.runningSpeed;
                    var sprintSpeedBonus = _moveSpeedBonusPercentage * speed.sprintSpeed;
                    
                    moveSpeed = Mathf.Lerp(moveSpeed, isSprinting ? speed.sprintSpeed+sprintSpeedBonus : speed.runningSpeed+runningSpeedBonus,
                        speed.movementSmooth * Time.fixedDeltaTime);
                }
            }
            else
