...
namespace Invector.vCharacterController.vActions
{
    ...
    public class vGenericAction : vActionListener
    {
        ...
        [vEditorToolbar("Settings")] [Tooltip("Tag of the object you want to access")]
        public string actionTag = "Action";

        [Tooltip("Use root motion of the animation")]
        public bool useRootMotion = true;

        [vEditorToolbar("Debug")] [Header("--- Debug Only ---")] [Tooltip("Check this to enter the debug mode")]
        public bool debugMode;

        [vReadOnly] protected vTriggerGenericAction _triggerAction;

        public virtual vTriggerGenericAction triggerAction
        {
            get => _triggerAction;
            set => _triggerAction = value;
        }

        [vReadOnly, SerializeField] protected bool _playingAnimation;
        [vReadOnly, SerializeField] protected bool actionStarted;
        [vReadOnly] public bool isLockTriggerEvents;
        [vReadOnly, SerializeField] protected List<Collider> colliders = new List<Collider>();

        [vEditorToolbar("Events")] public vOnActionHandle OnDoAction = new vOnActionHandle();
        ...
        protected bool finishLookAtMatch;
        ...
        protected virtual bool inActionAnimation
        {
            get
            {
                return !string.IsNullOrEmpty(triggerAction.playAnimation)
                       && tpInput.cc.animatorStateInfos.stateInfos[triggerAction.animatorLayer].shortPathHash
                           .Equals(Animator.StringToHash(triggerAction.playAnimation));
            }
        }
            ...
            foreach (var key in actions.Keys)
            {
                if (key)
                {
                    try
                    {
                        ...
                        if (mainCamera)
                        {
                            bool isValid = action.enabled && action.gameObject.activeInHierarchy &&
                                           (!action.activeFromForward &&
                                            (screenP - screenCenter).magnitude < distance ||
                                            IsInForward(action.transform, action.forwardAngle) &&
                                            (screenP - screenCenter).magnitude < distance);
                            if (isValid)
                            {
        ...
        protected virtual void AnimationBehaviour()
        {
            ...
            if (playingAnimation)
            {
                ...
                else if (triggerAction.useTriggerLookAt)
                {
                    EvaluateToTargetLookAt();
                }


                if (actionStarted && !triggerAction.endActionManualy &&
                    (triggerAction.inputType != vTriggerGenericAction.InputType.GetButtonTimer ||
                     !triggerAction.playAnimationWhileHoldingButton) &&
                    tpInput.cc.animatorStateInfos.GetCurrentNormalizedTime(triggerAction.animatorLayer) >=
        ...
        protected virtual void EvaluateToTargetPosition()
        {
            ...
            float normalizedTime =
                Mathf.Clamp(tpInput.cc.animatorStateInfos.GetCurrentNormalizedTime(triggerAction.animatorLayer), 0, 1);
        ...
        protected virtual void EvaluateToTargetLookAt()
        {
            Transform target = triggerAction.matchTarget;
            AnimationCurve rotationCurve = triggerAction.matchLookAtCurve;
            float normalizedTime = tpInput.cc.animatorStateInfos.GetCurrentNormalizedTime(triggerAction.animatorLayer);
            float evaluatedCurve = rotationCurve.Evaluate(normalizedTime);
            ...
            if (evaluatedCurve < 1f)
            {
                transform.rotation = Quaternion.Slerp(currentRotation, targetRotation, evaluatedCurve);
                finishLookAtMatch = true;
            }
            else if (finishLookAtMatch)
            {
        ...
        public virtual bool playingAnimation
        {
            ...
            protected set { _playingAnimation = true; }
        }
        ...
        public virtual bool actionConditions
        {
            get
            {
                return (!doingAction && !playingAnimation && !tpInput.cc.isJumping &&
                        !tpInput.cc.customAction /*&& !tpInput.cc.animator.IsInTransition(triggerAction.animatorLayer)*/
                    );
            }
        }
                ...
                if (!actions.ContainsKey(other))
                {
                    ...
                    for (int i = 0; i < _triggerActions.Length; i++)
                    {
                        ...
                        if (_triggerAction && _triggerAction.enabled)
                        {
                            ...
                            if (debugMode)
                            {
                                Debug.Log("<color=green>Enter in Trigger </color>" + other.gameObject,
                                    other.gameObject);
                            }
        ...
        public override void OnActionExit(Collider other)
        {
            ...
            if (other.gameObject.CompareTag(actionTag) && actions.ContainsKey(other) &&
                (!doingAction || other != triggerAction._collider))
            {
        ...
        public virtual void TriggerActionInput()
        {
            if (triggerAction == null || !triggerAction.gameObject.activeInHierarchy ||
                (triggerAction.CanDoAction == false))
            {
                return;
            }
            ...
            else if (triggerAction.inputType == vTriggerGenericAction.InputType.GetButtonDown && actionConditions)
            {
                ...
                if (tpInput.playerInput.actions["Interact"].WasPerformedThisFrame())
                {
                    TriggerActionEvents();
                    TriggerAnimation();
                }
        ...
        public virtual void TriggerAnimation()
        {
            ...
            if (triggerAction.animatorActionState != 0)
            {
                if (debugMode)
                {
                    Debug.Log($"<b>GenericAction: </b>Applied ActionState: " + triggerAction.animatorActionState + " ",
                        gameObject);
                }
            ...
            if (!string.IsNullOrEmpty(triggerAction.playAnimation))
            {
                if (!actionStarted)
                {
                    if (debugMode)
                    {
                        Debug.Log($"<b>GenericAction: </b>PlayAnimation: " + triggerAction.playAnimation + " ",
                            gameObject);
                    }
                    ...
                    tpInput.cc.animator.CrossFadeInFixedTime(triggerAction.playAnimation,
                        triggerAction.crossFadeTransition,
                        triggerAction.animatorLayer); // trigger the action animation clip
                    if (!string.IsNullOrEmpty(triggerAction.customCameraState))
                    {
                        tpInput.ChangeCameraState(triggerAction.customCameraState,
                            true); // change current camera state to a custom
                    }
                }
