    ...
    public abstract class vThrowManagerBase : vMonoBehaviour
    {
        ...
        protected virtual IEnumerator Start()
        {
            ...
            if (transform.root.TryGetComponent(out vDrawHideMeleeWeapons drawHide))
            {
                ...
                //onCancelAim.AddListener(() => drawHide.DrawWeapons());
            }
        ...
        protected virtual void UpdateThrowInput()
        {
            ...
            if (tpInput.playerInput.actions["Grenade"].WasPerformedThisFrame() && !inEnterThrowMode && !isThrowing && !isAiming)
            {
                EnterThrowMode();
                return;
            }
            ...
            if (tpInput.playerInput.actions["Grenade"].WasReleasedThisFrame() && aimHoldingButton && (isAiming || inEnterThrowMode) && !isThrowing)
            {
                ExitThrowMode();
            }
            ...
            if (isAiming && !isThrowing && !pressThrowInput)
            {
                ...
                if (tpInput.playerInput.actions["Shoot"].WasPerformedThisFrame())
                {
                    pressThrowInput = true;
                }
            }

            if (!aimHoldingButton && tpInput.playerInput.actions["Grenade"].WasPerformedThisFrame() && !pressThrowInput && (isAiming || inEnterThrowMode) && !isThrowing)
            {
                ExitThrowMode();
            }
        }
