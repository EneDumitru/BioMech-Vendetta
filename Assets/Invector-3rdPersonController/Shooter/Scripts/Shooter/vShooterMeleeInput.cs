    ...
    public class vShooterMeleeInput : vMeleeCombatInput, vIShooterIKController, PlayerController.vILockCamera
    {
        ...
        public virtual bool isAimingByHipFire
        {
            get
            {
                ...
                return shooterManager.hipfireShot && ((_aimTiming > 0 || (playerInput.actions["Shoot"].IsPressed() && shooterManager.CurrentWeapon != null)) || (!isAimingByInput && shootCountA > 0));
        ...
        public virtual void AimInput()
        {
            ...
            if (shooterManager.hipfireShot && !LockHipFireAiming)
            {
                ...
                if (playerInput.actions["Sprint"].WasPressedThisFrame() && _aimTiming > 0f)//sprintInput.GetButtonDown() 
                {
                    _aimTiming = 0f;
                }
            }
            ...
            if (!cc.isRolling)
            {
                isAimingByInput = (!isReloading || shooterManager.keepAimingWhenReload) && (playerInput.actions["Aim"].IsPressed()
                    || (shooterManager.alwaysAiming && CurrentActiveWeapon)) && !cc.ragdolled && !cc.customAction
                    || (cc.customAction && cc.isJumping);
            }

            //if (!cc.isRolling)
            //{
            //    isAimingByInput = (!isReloading || shooterManager.keepAimingWhenReload) && (aimInput.GetButton()
            //        || (shooterManager.alwaysAiming && CurrentActiveWeapon)) && !cc.ragdolled && !cc.customAction
            //        || (cc.customAction && cc.isJumping);
            //}


            if (playerInput.actions["Aim"].WasReleasedThisFrame() && !playerInput.actions["Shoot"].IsPressed())
            {
                _aimTiming = 0f;
            }
        ...
        public virtual void ShotInput()
        {
            ...
            if (IsAiming && !shooterManager.isShooting && aimConditions)
            {
                ...
                {
                    HandleShotCount(shooterManager.CurrentWeapon, playerInput.actions["Shoot"].IsPressed());
        ...
        public virtual void ReloadInput()
        {
            ...
            if (playerInput.actions["Reload"].WasPressedThisFrame()) //reloadInput.GetButtonDown()
            {
                shootCountA = 0;
                _aimTiming = 0f;
                ...
                if (CurrentActiveWeapon.autoReload && !shooterManager.WeaponHasLoadedAmmo() && shooterManager.WeaponHasUnloadedAmmo())
                {
                    switch (CurrentActiveWeapon.autoReloadStyle)
                    {
                        ...
                        case vShooterWeapon.AutoReloadStyle.WhenShot:
                            if (playerInput.actions["Shoot"].WasPressedThisFrame()) shooterManager.ReloadWeapon();
        ...
        public virtual void ScopeViewInput()
        {
            ...
            if (isAimingByInput && aimConditions && (playerInput.actions["Scope"].WasPressedThisFrame() || CurrentActiveWeapon.onlyUseScopeUIView))//scopeViewInput.GetButtonDown()
            {
        ...
        public override void UpdateCameraStates()
        {
            ...
            if (tpCamera == null)
            {
                tpCamera = GetComponentInChildren<vCamera.vThirdPersonCamera>();
                if (tpCamera == null)
                {
                    return;
                }
            ...
            else if (cc.isSwimming)
            {
                tpCamera.ChangeState("Swim", true);
            }
            else
            {
