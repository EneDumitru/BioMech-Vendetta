...
namespace Invector.vShooter
{
    ...
    public class vProjectileControl : vMonoBehaviour
    {
        ...
        internal bool hasCriticalDamage;
        internal float criticalDamageChange;
        internal float criticalDamageMultiplier;
        internal float _lowHpMinDamageBonus;
        internal float _lowHpMaxDamageBonus;
        ...
        protected virtual void Update()
        {
            ...
            if (Physics.Linecast(previousPosition, transform.position + transform.forward * 0.5f, out hitInfo, hitLayer))
            {
                ...
                if (!(ignoreTags.Contains(hitInfo.collider.gameObject.tag) || (shooterTransform != null && hitInfo.collider.transform.IsChildOf(shooterTransform))))
                {
                    ...
                    var isCrit = false;
                    if (hasCriticalDamage && Random.value <= criticalDamageChange)
                    {
                        isCrit = true;
                        damage.damageValue *= criticalDamageMultiplier;
                    }

                    var enemy = hitInfo.collider.gameObject.GetComponent<EmeraldAI.EmeraldAISystem>();
                    if(enemy != null)
                    {
                        enemy.HitPointTransform.position = hitInfo.point;
                        enemy.Damage((int)damage.damageValue, EmeraldAI.EmeraldAISystem.TargetType.Player,null,100,isCrit);
                    }
