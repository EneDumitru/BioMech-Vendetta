...
namespace Invector.vShooter
{
    ...
    public class vShooterWeaponBase : vMonoBehaviour
    {
        ...
        public float criticalDamageChange;
        ...
        private float _lowHpMinDamageBonus;
        private float _lowHpMaxDamageBonus;
        private bool _lowHpMode;
        #endregion
        ...
        private void Awake()
        {
            _healthController = GameManager.Instance.playerInstance.GetComponent<vHealthController>();
            ...
            foreach (var skillData in SkillsManager.Instance.SkillsData)
            {
                ...
                if (skillData.Category == SkillData.SkillCategory.Combat)
                {
                    switch (skillData.CombatType)
                    {
                        case SkillData.CombatSkillType.None:
                            break;
                        case SkillData.CombatSkillType.DamageBonus:
                            var bonusMin = (skillData.BonusValue / 100 * _minDamage);
                            if (bonusMin < 1)
                                _minDamage += 1;
                            else
                                _minDamage += (int)bonusMin;

                            var bonusMax = (skillData.BonusValue / 100 * _maxDamage);
                            if (bonusMax < 1)
                                _maxDamage += 1;
                            else
                                _maxDamage += (int)bonusMax;
                            
                            break;
                        case SkillData.CombatSkillType.AttackSpeedBonus:
                            _shootFrequency += (int)(skillData.BonusValue / 100 * _shootFrequency);
                            break;
                        case SkillData.CombatSkillType.CriticalDamage:
                            hasCriticalDamage = true;
                            criticalDamageMultiplier += skillData.BonusValue / 100 * criticalDamageMultiplier;
                            break;
                        case SkillData.CombatSkillType.DamageHpBelow:
                            _lowHpMode = true;
                            _lowHpMinDamageBonus = skillData.BonusValue / 100 * _minDamage;
                            _lowHpMaxDamageBonus = skillData.BonusValue / 100 * _maxDamage;
                            break;
                    }
                }
            }
        ...
        protected virtual void ShootBullet(Vector3 startPoint, Vector3 endPoint)
        {
            ...
            if (dispersion > 0 && projectile)
            {
                for (int i = 0; i < projectilesPerShot; i++)
                {
                    ...
                    var tempMaxDamage = maxDamage;
                    var tempMinDamage = minDamage;
                    
                    if (_healthController.isLowHp && _lowHpMode)
                    {
                        tempMaxDamage += (int)_lowHpMaxDamageBonus;
                        tempMinDamage += (int)_lowHpMinDamageBonus;
                    }
                    ...
                    pCtrl.maxDamage = (int)((tempMaxDamage / projectilesPerShot) * damageMultiplier);
                    pCtrl.minDamage = (int)((tempMinDamage / projectilesPerShot) * damageMultiplier);
                    pCtrl.minDamageDistance = minDamageDistance;
                    pCtrl.maxDamageDistance = maxDamageDistance;
                    pCtrl.hasCriticalDamage = hasCriticalDamage;
                    pCtrl.criticalDamageChange = criticalDamageChange;
                    pCtrl.criticalDamageMultiplier = criticalDamageMultiplier;
