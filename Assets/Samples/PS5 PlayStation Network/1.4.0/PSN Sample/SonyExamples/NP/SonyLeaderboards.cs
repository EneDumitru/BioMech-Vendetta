        ...
        public void MenuLeaderboards(MenuStack menuStack)
        {
            ...
            if (m_MenuLeaderboards.AddItem("Record Score (Large Data)", "Record a score on the board plus large data", enabled))
            {
                ...
                var requestOp = new AsyncRequest<Leaderboards.RecordScoreRequest>(request).ContinueWith((antecedent) =>
                {
                    ...
                    {
                        const uint SCE_NP_WEBAPI_SERVER_ERROR_LEADERBOARDS_LARGE_DATA_EXCEEDS_NUMBER_LIMIT = 0x8222F408;
                        if((uint)antecedent.Request.Result.sceErrorCode == SCE_NP_WEBAPI_SERVER_ERROR_LEADERBOARDS_LARGE_DATA_EXCEEDS_NUMBER_LIMIT)
                        {
                            /*
                             * https://game.develop.playstation.net/resources/documents/WebAPI/1/Leaderboards_WebAPI-Reference/0002.html
                             *
                             * Scores for which large-volume attachment data can be recorded are determined by the floor
                             * rank at which large-volume attachment data can be recorded (largeDataNumLimit) set for the leaderboard.
                             * For example, if largeDataNumLimit=10, places 1–10 can be recorded, and trying to record
                             * position 11 will result in an error and HTTP status 400 will be returned.
                             *
                             */

                            OnScreenLog.AddWarning("Error SCE_NP_WEBAPI_SERVER_ERROR_LEADERBOARDS_LARGE_DATA_EXCEEDS_NUMBER_LIMIT (0x8222F408) has occured when uploading the data");
                            OnScreenLog.Add("\n The position in the leaderboard is not valid for the submission of large data because it exceeds the rank required to submit large data.\n " +
                                "This is an expected error if the rank of the submitted score is greater than the largeDataNumLimit for the board\n", Color.green, keepNewLines: true);
                        }
                    }
