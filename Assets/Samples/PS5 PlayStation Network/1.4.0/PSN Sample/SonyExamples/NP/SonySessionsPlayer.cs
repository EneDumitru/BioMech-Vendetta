...
namespace PSNSample
{
    ...
    public partial class  SonySessions : IScreen
    {
        ...
        void DoPSUpdateSessionButtons(int currentUserId, PlayerSession currentPS)
        {
            ...
            String newleaderPlayerId = "nonPsnPlayer01";
            if (m_MenuSessions.AddItem("Change Non-PSN Leader", "Change player session leader to a Non-PSN player"))
            {
                AsyncOp requestOp = ChangeNonPsnLeader(currentUserId, newleaderPlayerId, currentPS);

                SessionsManager.Schedule(requestOp);

                OnScreenLog.AddNewLine();
                OnScreenLog.Add($"Changing player session leader to non-PSN player:{newleaderPlayerId}");
            }
        ...
        AsyncOp ChangeNonPsnLeader(Int32 currentUserId, String newLeaderPlayerId, PlayerSession currentPS)
        {
            PlayerSessionRequests.ChangePlayerSessionNonPsnLeaderRequest request = new PlayerSessionRequests.ChangePlayerSessionNonPsnLeaderRequest()
            {
                UserId = currentUserId,
                SessionId = currentPS.SessionId,
                PlayerId = newLeaderPlayerId,
            };

            var requestOp = new AsyncRequest<PlayerSessionRequests.ChangePlayerSessionNonPsnLeaderRequest>(request).ContinueWith((antecedent) =>
            {
                if (SonyNpMain.CheckAysncRequestOK(antecedent))
                {
                    OnScreenLog.Add("Leader set");
